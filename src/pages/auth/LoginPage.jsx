    ...
    return (
        <div style={{
            ...
            <div className="md:hidden" style={{
                ...
                <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-16)' }}>
                    <div style={{
                        ...
                        background: 'rgba(255,255,255,0.2)', borderRadius: 'var(--radius-xl)',
                ...
                <p style={{ fontSize: 'var(--font-size-lg)', opacity: 0.8, maxWidth: '400px', lineHeight: 'var(--line-height-relaxed)' }}>
                    Share updates and get approvals from clients in one clean, professional portal.
                </p>
                <div style={{ marginTop: 'var(--space-12)', display: 'flex', gap: 'var(--space-8)' }}>
                    {[['Clients', 'Managed'], ['Updates', 'Shared'], ['Approvals', 'Tracked']].map(([val, label]) => (
                        <div key={label}>
                            <div style={{ fontSize: 'var(--font-size-2xl)', fontWeight: 'var(--font-weight-bold)' }}>✓</div>
                            <div style={{ fontSize: 'var(--font-size-sm)', opacity: 0.7 }}>{val} {label}</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div style={{
                ...
                {/* Mobile logo */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-8)' }} className="md:flex">
                    <div style={{
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                    <div className="form-group">
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input type="email" className="form-input" placeholder="you@agency.com" value={email} onChange={e => setEmail(e.target.value)} style={{ paddingLeft: '44px' }} required />
                        </div>
                    </div>
                ...
                <p style={{ marginTop: 'var(--space-6)', textAlign: 'center', fontSize: 'var(--font-size-sm)', color: 'var(--text-tertiary)' }}>
                    Don't have an account?{' '}
                    <Link to="/signup" style={{ color: 'var(--color-primary-600)', fontWeight: 'var(--font-weight-medium)' }}>Create one free</Link>
                </p>
                <p style={{ marginTop: 'var(--space-3)', textAlign: 'center', fontSize: 'var(--font-size-sm)', color: 'var(--text-tertiary)' }}>
                    Are you a client?{' '}
                    <Link to="/client-login" style={{ color: 'var(--color-primary-600)', fontWeight: 'var(--font-weight-medium)' }}>Client portal →</Link>
                </p>
            </div>
        </div>
    );
